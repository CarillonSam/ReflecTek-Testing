...
LOCAL_FILE_HB = r"C:\Users\uconn\Downloads\ReflecTekCalibrationScholl\Array-Calibration\src\DataHB.txt"   #HB voltage file to send to PI
LOCAL_FILE_LB = r"C:\Users\uconn\Downloads\ReflecTekCalibrationScholl\Array-Calibration\src\DataLB.txt" #LB voltage file to send to PI
...
def update_lb_array_file(V):
    ...
    with open(LOCAL_FILE_LB, "w") as f:#should be LOCAL_FILE_LB
        for row in V:
            line = ",".join(str(x) for x in row)
            f.write(line + "\n")
    #This program is for LB only, so create a 0V array for the high band which is 24x8 in Sam's code
    with open(LOCAL_FILE_HB, "w") as f:#should be LOCAL_FILE_HB
